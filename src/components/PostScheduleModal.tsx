...
export default function PostScheduleModal({
  ...
  // Calculate the next 10 free slots in Europe/Berlin (06:00 & 20:00)
  const previewSlots = useMemo(
    () => findNextFreeBerlinSlots(existingPosts, 10),
    [existingPosts]
  );
        ...
        <div className="flex items-start justify-between gap-4 border-b border-coal-700/70 pb-4">
          <div>
            ...
            <p className="mt-0.5 font-mono text-[11px] text-coal-300">
              Plant automatisch jeweils 2 Slots pro Tag (06:00 Uhr & 20:00 Uhr) über die nächsten 5 freien Tage. Belegte Slots werden intelligent übersprungen.
            </p>
          </div>
          <button
            type="button"
        ...
        {successResult ? (
          <div className="py-6 text-center">
            ...
            <h4 className="mt-4 font-display text-xl font-black uppercase text-paper-100">
              {successResult.count} Posts erfolgreich für 06:00 & 20:00 Uhr eingeplant!
            </h4>
          ...
          <div className="mt-5 grid gap-5">
            ...
            <div>
              <div className="mb-2 flex items-center justify-between">
                <span className="mono-label flex items-center gap-1.5 text-[9.5px] text-volt-300">
                  <Clock className="size-3.5" /> INTELLIGENTE SLOT-VORSCHAU (NÄCHSTE 10 FREIE SLOTS)
                </span>
            ...
            <div className="flex flex-wrap items-center justify-end gap-3 border-t border-coal-700/70 pt-4">
              ...
              >
                {submitting ? (
                  <>
                    <Loader2 className="size-4 animate-spin" /> Plane 10 Posts über Zernio…
                  </>
                ) : (
                  <>
                    <Send className="size-4" /> Jetzt 10 Posts planen (06:00 & 20:00 Uhr)
                  </>
                )}
              </button>
            </div>
